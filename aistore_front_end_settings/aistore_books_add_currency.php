...
function aistore_add_currency(){
      ...
      //  echo "<script>alert('Add Currency Successfully');</script>";
	...
	toastr.success('Add Currency Successfully');
	</script>
<?php
    $message='Add Currency Successfully';
            sendNotification($cid,$message);   
            
            
            $aistore_add_payment_url = esc_url(add_query_arg(array(
                    'page_id' => get_option('aistore_payment_instructions') ,
                ) , home_url()));
                
            ?>
    <meta http-equiv="refresh" content="0; URL=<?php echo esc_html($aistore_add_payment_url); ?>" /> 
...
?>
  ...
  <td> 		   <?php echo esc_attr($row->symbol); ?> </td>
   <td><?php
                if (isset($_POST['submit']) and $_POST['action'] == 'aistore_currency')
                {
                    ...
                    $table = $wpdb->prefix . 'aistore_books_currency';
                ...
                {
?>
<form method="POST" action="" name="aistore_currency" enctype="multipart/form-data"> 

<?php wp_nonce_field('aistore_nonce_action', 'aistore_nonce'); ?>
	<input 
 type="hidden" name="currency_id" value="<?php echo esc_attr($row->id); ?>"/>
<input 
 type="submit" name="submit" value="<?php _e('Delete', 'aistore') ?>"/>
<input type="hidden" name="action" value="aistore_currency" />
                </form><?php
                } ?></td>
            </tr>
    <?php
