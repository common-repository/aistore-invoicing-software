...
<?php
    ...
    global $wpdb;
           ...
           $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}aistore_books_product ( name,short_description, amount, price,full_description,user_id,category,terms_condtion,tags,product_type,company_id ) VALUES (%s ,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", array(
            $name,
            $short_description,
            $amount,
            $price,
...
<?php
    ...
    $aistore_add_vendor_url = esc_url(add_query_arg(array(
                    'page_id' => get_option('aistore_add_vendor'),
                ) , home_url()));
          
            ?>
 <meta http-equiv="refresh" content="0; URL=<?php echo esc_html($aistore_add_vendor_url); ?>" /> 
