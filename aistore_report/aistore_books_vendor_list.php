...
$user_id = get_current_user_id();
  ...
  if (isset($_POST['submit']) and $_POST['action'] == 'delete_vendor')
    ...
    $table = $wpdb->prefix.'aistore_books_vendor';
    $wpdb->delete( $table, array( 'id' => $vendor_id,'user_id' =>$user_id, 'company_id' => $company->id) );
        }
...
?>    <tr>
		   ...
		   <td> <a href="<?php echo $details_vendor_page_id_url; ?>"  ><?php echo esc_attr($row->id); ?> </a></td>
		   
		    <td>  <a href="<?php echo $transaction_vendor_page_id_url; ?>" >
		    <?php echo esc_attr($row->vendor_name); ?></a> </td>
    <td> 	
 
   <?php echo esc_attr($row->vendor_email) ?>
		  </td>
...
<?php wp_nonce_field('aistore_nonce_action', 'aistore_nonce'); ?>
  <input class="input" type="hidden" id="vendor_id" name="vendor_id" value="<?php echo esc_attr($row->id); ?>"><br>
