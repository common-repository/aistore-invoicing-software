...
function aistore_add_vendor(){
       ...
       if (isset($_POST['submit']) and $_POST['action'] == 'vendor_system')
        {
           ...
           $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}aistore_books_vendor ( vendor_name,vendor_email, user_id,vendor_type,mobile_no,first_name,last_name,country,address,city,state,zip_code ,pan_number,gst_number,company_id) VALUES (%s ,%s,%s,%s ,%s,%s,%s ,%s,%s,%s ,%s,%s,%s,%s,%s)", array(
            $vendor_name,
            $vendor_email,
            $user_id,
            $vendor_type,
	...
	toastr.success('Add Vendor Successfully');
	</script>
<?php

 $message='Add Vendor Successfully';
            sendNotification($id,$message); 
            $aistore_add_customer_url = esc_url(add_query_arg(array(
                    'page_id' => get_option('aistore_add_customer') ,
                ) , home_url()));
            
               ?>
        
    <meta http-equiv="refresh" content="0; URL=<?php echo esc_html($aistore_add_customer_url); ?>" />  
