...
<?php
 ...
 global $wpdb;
$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}aistore_books_estimate (estimate_id, ship_to,amount, user_id ,description,product_id,customer_id,bill_to,currency,fee,company_id) VALUES (%s, %s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", array(
            $estimate_id,
            $ship_to,
            $amount,
            $user_id,
	...
	toastr.success('Add Estimate Successfully');
	</script>
<?php

               $message='Add Estimate Successfully';
...
<label><?php echo esc_attr($payment->payment_instructions) ; ?></label><br/>
   ...
   <div class="mb-3">   
<label><?php _e('Estimate Id', 'aistore'); ?></label><br/>
  <input class="input" type="text" id="estimate_id" name="estimate_id" required></div><br><br>
    <br>   
...
?>
  ...
  <?php 
   ...
   ?>
    ...
    <div class="mb-3">   
<label><?php _e('Customer', 'aistore'); ?></label>
  <?php $aistore_add_product_page_id_url = esc_url(add_query_arg(array(
